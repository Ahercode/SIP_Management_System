...
const Roaster = () => {
  ...
  const deleteData = async (element: any) => {
    try {
      const response = await axios.delete(`${Api_Endpoint}/Roasters/${element.id}`)
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const response = await axios.get(`${Api_Endpoint}/Roasters`)
  ...
  const { isLoading, mutate: updateData } = useMutation(updateItem, {
    onSuccess: (data) => {
      queryClient.setQueryData(['Roasters', tempData.id], data);
      reset()
      setTempData({})
      loadData()/*  */
      setIsUpdateModalOpen(false)
  ...
  const url = `${Api_Endpoint}/Roasters`

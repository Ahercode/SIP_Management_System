...
import { EmployeeProfile } from '../../employee/Employee'
...
const AppraisalPerformance = () => {
  ...
  const [referenceId, setReferenceId] = useState<any>(
    `${selectedPaygroup}-${selectedAppraisalType}-${selectedStartPeriod}-${selectedEndPeriod}`
  )
  ...
  const {data: allAppraisals} = useQuery(
    'appraisals',
    () => fetchDocument(`appraisals/tenant/${tenantId}`),
    {cacheTime: 10000}
  )
  const {data: allPeriods} = useQuery('periods', () => fetchDocument(`periods`), {cacheTime: 10000})
  const {data: allJobTitles} = useQuery(
    'jobTitles',
    () => fetchDocument(`jobTitles/tenant/${tenantId}`),
    {cacheTime: 10000}
  )
  ...
  const {data: allParameters} = useQuery(
    'parameters',
    () => fetchDocument(`parameters/tenant/${tenantId}`),
    {cacheTime: 10000}
  )
  const {data: allAppraTranItems} = useQuery(
    'appraisalPerItems',
    () => fetchDocument(`AppraisalPerItems`),
    {cacheTime: 10000}
  )
  const {data: allAppraisalsPerfTrans} = useQuery(
  ...
  const activeReferenceId = activeReference?.find((item: any) => {
  ...
  const checkActive = allReviewdates?.data?.find((item: any) => {
    return (
      item?.isActive?.trim() === 'active' && item?.referenceId === activeReferenceId?.referenceId
    )
  })
  ...
  function handleDelete() {
    selectedRowKeys.map((del: any) => {
      const recordId = allAppraisalTranItems.filter((item: any) => {
        return (
          parseInt(item.employeeId) === del &&
          item.appraisalPerfTranId === parseInt(selectedReference)
        )
      }, [])
    ...
    },
    {
      ...
      render: (row: any) => {
        return (
          parseFloat(
            getOverallAchievement({
              parameterData: allParameters?.data,
  ...
  if (checkActive === undefined || checkActive?.tag?.trim() === 'setting') {
    columns.splice(6, 1)
  }
  ...
  const submitApplicant = handleSubmit(async (values) => {
    ...
    const result = allAppraisalTranItems?.filter((item: any) => {
      return (
        parseInt(item.employeeId) === employeeRecord.id &&
        item.appraisalPerfTranId === parseInt(selectedReference)
      )
    })
    ...
    if (reference?.length > 0) {
      ...
      if (result?.length > 0) {
        ...
        try {
          axios
            .post(`${Api_Endpoint}/${url}`, dataToPost)
            .then((res) => {
    ...
    <div>
        ...
        <>
          <div style={{padding: '20px 0px 0 0px'}} className='col-12 row mb-0'>
            <div className='col-3 mb-7'>
              ...
              <input
                readOnly
                value={getFieldName(appraisalTranItem?.paygroupId, allPaygroups?.data)}
            ...
            <div className='col-3 mb-7'>
              ...
              <input
                readOnly
                value={getFieldName(appraisalTranItem?.appraisalTypeId, allAppraisals?.data)}
                className='form-control form-control-solid'
              />
            </div>
            <div className='col-3 mb-7'>
              ...
              <input
                readOnly
                value={getFieldName(appraisalTranItem?.startPeriod, allPeriods?.data)}
                className='form-control form-control-solid'
              />
            </div>
            <div className='col-3 mb-7'>
              ...
              <input
                readOnly
                value={getFieldName(appraisalTranItem?.endPeriod, allPeriods?.data)}
                className='form-control form-control-solid'
              />
            </div>
          </div>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key='submit' type='primary' htmlType='submit' onClick={submitApplicant}>
            Submit
          </Button>,
        ]}
      >
        <form onSubmit={submitApplicant}>
          ...
          <Table columns={columns} dataSource={empSelectedPaygroup} />
        </form>
      </Modal>
      {selectedReference === null || selectedReference === 'select reference' ? (
        ''
          ...
          <div className='d-flex justify-content-between'>
            ...
            <Space style={{marginBottom: 16}}>
              {selectedRowKeys?.length > 0 ? (
                ...
                <>
                  <button
                    style={{marginBottom: 16, backgroundColor: '#216741', color: '#f2f2f2'}}
                    type='button'
                    className='btn  me-3'
                    onClick={showEmpAddModal}
                  >
              ...
              <div style={{padding: '20px 20px 0 20px'}} className='row mb-0 '>
                ...
                <div className='col-6 mb-3'>
                  ...
                  <input
                    type='text'
                    readOnly
                    defaultValue={employeeRecord?.surname}
                    className='form-control form-control-solid'
                  />
                </div>
              </div>
              <div style={{padding: '20px 20px 10px 20px'}} className='row mb-7 '>
                <div className='col-6 mb-3'>
                  ...
                  <input
                    type='text'
                    readOnly
                    defaultValue={employeeRecord?.dob?.substring(0, 10)}
                    className='form-control form-control-solid'
                  />
                </div>
                <div className='col-6 mb-3'>
                  ...
                  <input
                    type='text'
                    readOnly
                    defaultValue={employeeRecord?.gender}
                    className='form-control form-control-solid'
                  />
                </div>
              </div>

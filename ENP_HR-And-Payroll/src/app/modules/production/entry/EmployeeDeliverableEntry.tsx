...
const EmployeeDeliverableEntry = () => {
  ...
  const sameObjective = appraisalobjectives?.data?.filter(
    (item: any) => item?.tag?.trim() === 'same' && item?.id.toString() === param.objectiveId
  )
  ...
  const loadData = async () => {
    try {
      const neewData = allObjectiveDeliverables?.data?.filter(
        (item: any) => item?.objectiveId?.toString() === param.objectiveId
      )
  ...
  const weightSum = (itemToPost: any) => {
    return allObjectiveDeliverables?.data
      .filter((item: any) => item.objectiveId === itemToPost.objectiveId)
  ...
  const handleUpdate = async (e: any) => {
    ...
    if (
      tempData.name === secondTempData.name &&
      tempData.description === secondTempData.description &&
      tempData.unitOfMeasureId === secondTempData.unitOfMeasureId &&
      tempData.target === secondTempData.target
    ) {
      ...
      const itemExists = gridData?.find(
        (item: any) =>
      ...
      if (itemExists) {
        return message.error('Deliverable already exists')
      } else {
        const item: any = {
          url: 'AppraisalDeliverable',
          data: tempData,
